...
@Component({
  ...
  template: `
      ...
      <div class="task-list" *ngIf="!loading()">
        <div class="task-row" *ngFor="let t of tasks()" [class.done]="isDone(t)">
          <div class="task-left">
            ...
            <div class="task-info">
              <div class="task-title" [class.done-text]="isDone(t)">{{ t.title }}</div>
    ...
    <div class="modal" *ngIf="showAdd">
      ...
      <div class="field"><label>Task Title *</label><input [(ngModel)]="form.title" class="inp" placeholder="e.g. Take blood pressure reading" /></div>
      <div class="field"><label>Description</label><textarea [(ngModel)]="form.description" class="inp ta" rows="3" placeholder="Instructions for the patient..."></textarea></div>
      ...
      <div class="modal-btns">
        ...
        <button class="mbtn-yes" (click)="submit()" [disabled]="!form.title || adding()">
...
export class PatientTasksComponent implements OnInit {
  ...
  tasks   = signal<any[]>([]);
  showAdd = false;
  form: any = { title:'', description:'', dueDate:'' };
  private patientId = '';
  ...
  submit(): void {
    ...
    const body: any = { patientId: this.patientId, title: this.form.title };
    if (this.form.description) body.description = this.form.description;
    if (this.form.dueDate) body.dueDate = this.form.dueDate;
    this.svc.addTask(body).subscribe({
      next: () => { this.adding.set(false); this.showAdd=false; this.form={title:'',description:'',dueDate:''}; this.load(); },
      error: (e: any) => { this.adding.set(false); this.err.set(e?.error?.message ?? 'Failed.'); }
    });
  }
