...
export class DoctorProfileComponent implements OnInit {
  ...
  onPhoto(e: Event): void {
    ...
    this.svc.uploadPicture(file).subscribe({
      next: (res: any) => {
        console.log('[doctorProfile] upload res:', res);
        const url =
        ...
        if (url) {
          console.log('[doctorProfile] photo URL:', url);
          this.avatarUrl.set(url);
          this.photoSvc.set(url);
          // Persist URL in profile
          this.svc.updateDoctorNurse({ profilePictureUrl: url, firstName: this.firstName||undefined, lastName: this.lastName||undefined, email: this.email||undefined, education: this.education||undefined, certifications: this.certifications||undefined }).subscribe({ next: () => console.log('[DoctorProfile] URL saved'), error: (e:any) => console.error('[DoctorProfile] save failed', e) });
          // Notify shell to update sidebar/header
          window.dispatchEvent(new CustomEvent('wateen:photo', { detail: url }));
        } else {
          console.warn('[doctorProfile] no URL in response — reloading profile');
          this.svc.getDoctorData().subscribe({ next: (r:any) => { const d=r?.data??r; const p=d?.profilePictureUrl??d?.avatarUrl??''; if(p) this.avatarUrl.set(p); }, error:()=>{} });
        }
        this.uploading.set(false);
      },
      error: (err: any) => {
        console.error('[doctorProfile] upload error:', err);
        this.uploading.set(false);
        this.avatarUrl.set('');
      }
    });
