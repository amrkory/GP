...
import { ApiResponse, AiChatResponse } from '../../../core/models/api.models';
...
@Component({
  ...
  template: `
    <div class="chat-page">
      ...
      <div class="disclaimer">
        ...
        This AI provides general health guidance only — not a substitute for medical advice.
      </div>
      ...
      <div class="messages" #msgContainer>
        <div class="msg-wrap assistant" *ngIf="messages().length === 0">
          <div class="msg-bubble assistant">
            ...
            I can help with symptom guidance, medications, nutrition, and finding the right specialist.
          </div>
        </div>
        ...
        <div class="msg-wrap" *ngFor="let m of messages()" [class]="m.role">
          <div class="msg-bubble" [class]="m.role">
            <span [innerHTML]="formatText(m.text)"></span>
  ...
  styles: [`
    ...
    .ai-status   { font-size:12px; color:#22c55e; }
    .disclaimer  { display:flex; align-items:flex-start; gap:6px; background:#FEF9E7; border-bottom:1px solid #FDE68A; padding:8px 16px; font-size:12px; color:#92400E; flex-shrink:0; }
    ...
    .msg-bubble.assistant { background:#fff; color:#111; border-bottom-left-radius:4px; box-shadow:0 1px 4px rgba(0,0,0,0.07); }
    ...
    .quick-prompts { display:flex; flex-direction:column; gap:6px; align-self:flex-start; }
    .quick-btn   { background:#fff; border:1.5px solid #e8e8e8; border-radius:20px; padding:8px 14px; font-size:13px; color:#555; cursor:pointer; font-family:'Cairo',sans-serif; }
...
export class AiAssistantComponent implements AfterViewChecked {
  ...
  sendMessage(text?: string): void {
    ...
    this.http.post<ApiResponse<AiChatResponse>>(
      `${environment.apiUrl}/ai/chat`,
      { message: msg, conversationId: this.conversationId || undefined }
    ).subscribe({
      next: (res: ApiResponse<AiChatResponse>) => {
        this.conversationId = res.data.conversationId;
        const replyText = res.data.reply + (res.data.disclaimer ? `\n\n_${res.data.disclaimer}_` : '');
        this.messages.update(m => [...m, { role: 'assistant', text: replyText, time: new Date() }]);
        this.thinking.set(false);
        this.shouldScroll = true;
      },
      error: () => {
        this.messages.update(m => [...m, { role: 'assistant', text: 'Sorry, I had trouble responding. Please try again.', time: new Date() }]);
        this.thinking.set(false);
        this.shouldScroll = true;
      },
    });
  ...
  formatText(text: string): string {
    return text.replace(/\n/g, '<br>').replace(/_(.+?)_/g, '<em>$1</em>').replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>');
  }
