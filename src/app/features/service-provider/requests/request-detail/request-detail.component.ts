  ...
  template: `
    <div class="page">
      ...
      <div class="not-found" *ngIf="!loading() && !req()">
        ...
        <button (click)="router.navigate(['/provider/requests'])">Back to requests</button>
      </div>
      <ng-container *ngIf="!loading() && req()">
        ...
        <div class="hero-card">
          ...
          <span class="status-pill" [class]="req()!.status?.toString().toLowerCase()">{{ req()!.status }}</span>
        </div>
        ...
        <div class="info-card">
          ...
          <div class="info-row">
            ...
            <div><div class="info-lbl">Scheduled</div><div class="info-val">{{ (req()!.requestedTime ?? req()!.scheduledAt ?? req()!.date) | date:'EEEE, MMMM d, y · h:mm a' }}</div></div>
          </div>
          <div class="info-row">
        ...
        <!-- Pending actions -->
        <div class="actions" *ngIf="(req()!.status === 'Pending' || req()!.status === 0)">
        ...
        <!-- Accepted actions -->
        <div class="actions" *ngIf="(req()!.status === 'Accepted' || req()!.status === 1)">
    ...
    <div class="dialog" *ngIf="showRejectDialog">
      ...
      <h3>{{ (req()?.status === 'Accepted' || req()?.status === 1) ? 'Cancel Visit?' : 'Reject Request?' }}</h3>
      <p>{{ (req()?.status === 'Accepted' || req()?.status === 1) ? 'Cancel visit for' : 'Reject request from' }}
...
export class RequestDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Load all nurse requests then find by id
    this.svc.getNurseRequests(1, 200).subscribe({
      next: (res: any) => {
